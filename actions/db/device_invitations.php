...
class Device_Invitations{
    ...
    // RETURN: boolean
    public function add_invitation($device_id, $user_id, $node_id, $expires, $is_valid){
        if ($stmt = $this->connection->prepare('INSERT INTO device_invitations(device_id, user_id, node_id, expires, is_valid) VALUES (?, ?, ?, ?, ?)')) {
            $stmt->bind_param('siiii', $device_id, $user_id, $node_id, $expires, $is_valid);
            $status = $stmt->execute();
            $stmt->close();
            if($status === false){
    ...
    public function get_registered_devices($node_id){
        if ($stmt = $this->connection->prepare('SELECT id, device_id, expires, is_valid FROM device_invitations WHERE node_id = ?')) {
